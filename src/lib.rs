...
impl scale_decode::visitor::Visitor for BorrowVisitor {
        ...
        match first {
            Some(Ok(Value::U8(_))) => {
                ...
                return Ok(Value::Scale(
                    &value.bytes_from_start()[1/* TODO this 1 is assuming the compact int of the len is only one byte long*/..=(value.remaining() + already_read_items)],
                ));
            }
    ...
    fn visit_bitsequence<'scale, 'm>(
        ...
        let mut bits = scale_bits::Bits::new(); //todo size
        for bit in value.decode().unwrap() {
            bits.push(bit.unwrap());
        }

        Ok(Value::Bits(Box::new(bits)))
    }
}
...
/// if optimise then 128bits put in 64bits where possible and owned slices of u8 are created.
/// If not optimise then we try and keep exactly the same shape as scale_value.
/// TODO: tying these lifetimes together is not ideal.
#[cfg(feature = "convert")]
pub fn convert<'scale, T>(
    input: &'scale scale_value::Value<T>,
    optimise: bool,
) -> crate::Value<'scale, 'scale> {
    match &input.value {
        scale_value::ValueDef::Primitive(p) => match &p {
            scale_value::Primitive::U128(uint) => {
                if optimise {
                    if let Ok(uu64) = u64::try_from(*uint) {
                        crate::Value::U64(uu64)
                    } else {
                        crate::Value::U128(Box::new(*uint))
                    }
                } else {
                    crate::Value::U128(Box::new(*uint))
                }
            }
            scale_value::Primitive::Bool(uint) => crate::Value::Bool(*uint),
            scale_value::Primitive::Char(uint) => crate::Value::Char(*uint),
            scale_value::Primitive::String(uint) => crate::Value::Str(uint.as_str()),
            scale_value::Primitive::U256(uint) => crate::Value::U256(uint),
            scale_value::Primitive::I256(uint) => crate::Value::I256(uint),
            scale_value::Primitive::I128(uint) => crate::Value::I128(Box::new(*uint)),
        },
        scale_value::ValueDef::Composite(comp) => convert_composite(comp, optimise),
        scale_value::ValueDef::Variant(var) => crate::Value::Object(Box::new(vec![(
            var.name.as_str(),
            convert_composite(&var.values, optimise),
        )])),
        scale_value::ValueDef::BitSequence(bits) => crate::Value::Bits(Box::new(bits.clone())),
    }
}

#[cfg(feature = "convert")]
fn convert_composite<T>(
    input: &'_ scale_value::Composite<T>,
    optimise: bool,
) -> crate::Value<'_, '_> {
    match input {
        scale_value::Composite::Named(comp) => {
            let mut result = vec![];
            for (name, c) in comp {
                result.push((name.as_str(), convert(c, optimise)));
            }
            crate::Value::Object(Box::new(result))
        }
        scale_value::Composite::Unnamed(comp) => {
            //Inefficient conversion but at least the result is closer to what we want.
            if optimise
                && comp.iter().all(|c| {
                    if let scale_value::ValueDef::Primitive(scale_value::Primitive::U128(cc)) =
                        c.value
                    {
                        u8::try_from(cc).is_ok()
                    } else {
                        false
                    }
                })
            {
                if comp.len() == 0 {
                    return crate::Value::Object(Box::new(vec![]));
                }
                ...
                let v: Vec<u8> = comp
                    ...
                    .collect();
                return crate::Value::ScaleOwned(Box::new(v));
            }

            let mut result = vec![];
            assert!(NUMS.len() > comp.len(), "need at least {}", comp.len());
            for (c, &i) in comp.iter().zip(NUMS).take(30) {
                result.push((i, convert(c, optimise)));
            }
            crate::Value::Object(Box::new(result))
        }
    }
...
mod tests {
    ...
    fn bool_test() {
        ...
        let val =
            decode_with_visitor(&mut encoded.as_slice(), id.id, &types, BorrowVisitor {}).unwrap();
